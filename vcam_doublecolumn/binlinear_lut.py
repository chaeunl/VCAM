...
class BinLinear_LUT(BinLinear): #inherits BinLinear with additional functions!
    ...
    def forward_lut(self, input, tune=False, bias_noise_std=0.0, same_seq_through_group=False):
        ...
        if tune:
            ...
            wns.add_( -sbias.clamp(max=0.0) ).clamp_(min=0.0, max=self.sWL).to(self.weight.device)
        ...
        with torch.cuda.device(self.weight.device):
            FFL.get_vout_from_lut(vout, wps, wns, lut_no, self.LUT)
            #print(x[0:5,0:5])

        #check error and post-process
        err = (vout.cpu() <= -50.0)
        if err.sum() > 0:
            raise Exception("cuda: Error @ {}, {}".format(vout.min(), vout.argmin()))

        #vout = vout.sum(2).div_(nCBA)
        vout = vout.sum(2) / nCBA

        if bias_noise_std > 0.0:
            vout.add_( vout.clone().normal_(0.0, bias_noise_std/sB) )
        
        #make op_amp vout.
        #vout.add_( -self.vref_base )
        vout = vout - self.vref_base
